const defaultConfiguration = {minimumReportInterval: 3, maximumReportInterval: 300, reportableChange: 0};
...
module.exports = {
    async remove(device) {
       ...
       this.debug('reporting remove', device.ieeeAddr);
        ...
        try {
            for (const endpoint of device.endpoints) {
                for (const [cluster, configuration] of Object.entries(clusters)) {
                    ...
                    if (endpoint.supportsInputCluster(cluster)) {
                        ...
                        const removeConfiguration = configuration.map(c => Object.assign({}, c, {maximumReportInterval: 0, minimumReportInterval: 0xffff}));
            ...
            this.error(`Failed to remove reporting for '${device.ieeeAddr}' - ${error.message}`);
        }

        delete device.meta.shouldReport;
        await device.save();
    ...
    async setup(device) {
        ...
        this.debug('reporting setup', device.ieeeAddr);
        ...
        try {
            ...
            this.error(`Failed to setup reporting for '${device.ieeeAddr}' - ${error.message}`);
        }

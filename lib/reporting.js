            ...
            for (const endpoint of device.endpoints) {
                for (const [cluster, configuration] of Object.entries(clusters)) {
                    ...
                    if (endpoint.supportsInputCluster(cluster)) {
                        this.log(`bind ${cluster} to coordinator`);
                        await this.bind(device.ieeeAddr, endpoint.ID, cluster, this.coordinatorEndpoint.deviceIeeeAddr, this.coordinatorEndpoint.ID);
